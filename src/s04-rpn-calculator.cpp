...
auto evaluate_rdiv(std::stack<double>& stack) -> void
{
    if (stack.size() < 2) {
        throw std::logic_error{"not enough operands for %"};
    }
...
auto evaluate_sum(std::stack<double>& stack) -> void  // sum - adds all items in
                                                      // stack
{
    if (stack.size() < 1) {
        throw std::logic_error{"not enough operands for sum"};
    }
    auto sum = 0;
    do {
        sum += pop_top(stack);
